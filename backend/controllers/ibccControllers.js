...
const { file } = require("googleapis/build/src/apis/file");
  ...
  exports.createNewSolo = async (req, res) => {
    try {
        ...
        const rootFolderId = process.env.FOLDER_BUSINESS_CASE_ID;
        const folderId = await createFolder("Individual - " + user.fullname, rootFolderId);
        ...
        const skillsQuestionnaire = [
            {
                "Have you participated in similar business case competitions before?":experience,
            },
            {
                "What are your main goals for participating in this business case competition?":goals,
            },
            {
                "Any other information about your background that would be helpful for matchmaking?":background
            }
        ];

        const commitmentQuestionnaire = [
            { "Are you willing to collaborate effectively with assigned teammates?": commitment1 },
            { "Are you willing to commit and participate in all scheduled rounds and activities of the competition?": commitment2 }
        ];
